...
public class FiguraRetas {
    ...
    public static void desenharReta(Graphics g, int x1, int y1, int x2, int y2, String nome, int esp, Color cor){
       reta = new RetaGr(x1, y1, x2, y2, cor, nome, esp);
       reta.desenharReta(g);
    }
    
    public static void desenharReta(Graphics g, RetaGr retax){
        retax.desenharReta(g);
    }
    ...
    public static void desenharRetas(Graphics g, int qtde, int esp){
        ...
        for(int i=0; i < qtde; i++) {
            ...
            Color cor = new Color((int) (Math.random() * 256),  
                    (int) (Math.random() * 256),  
                    (int) (Math.random() * 256));
            reta = new RetaGr(x1, y1, x2, y2, cor, "", esp);
            reta.desenharReta(g);
        }
    }
}
